function [xMR, iflag, total_iters, resMR] = lsqminres_sd(A,Q,R,f,a,tol,n_max,x0,ptype)
% LSQMINRES_SD     preconditioned MINRES algorithm in single, double precision
% [xMR, iflag, total_iters, resMR] = lsqminres_sd(A,Q,R,f,a,tol,n_max,x0)
...
A = single(A); Q = single(Q); R = single(R);
f = single(f); a = single(a);
