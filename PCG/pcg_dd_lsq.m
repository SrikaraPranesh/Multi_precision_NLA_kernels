function [x,nbe,iter,flag] = pcg_dd_lsq(A,x,b,L,tol)
%PCG_DQ_LSQ   Symmetric-preconditioned PCG in double precision
%   Solves (A^{T}A)x=b by solving the preconditioned linear system
%   L^{-T}L^{-1}(A^{T}A)x=L^{-T}L^{-1}b using the Precondition 
%   Conjugate Gradients ( PCG ) method.
%
%
