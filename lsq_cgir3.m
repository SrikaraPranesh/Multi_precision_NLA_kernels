function [x,its,t_pcg_its] = lsq_cgir3(A,b,precf,precw,precr,iter_max,gtol,scale)
%LSQ_CGIR3 Normal equation based least squares solver using CG-IR.
%   Solves the full rank least square problem using the normal equation
%   method. The normal equation is solved using CG-IR with low precision
%   Cholesky factors as preconditioners, and it also refines the solution.
%     iter_max ref. steps and CG convergence tolerance gtol, with
...
%
%   NOTE -- Refer to the documentation of gmresir3 for further information
%   on CG-IR call. By default no
%   plots of GMRES-IR are generated.
...
LL = (1/sqrt(mu))*diag(1./diag(S))*double(R');
...
for its = 1:iter_max(1,1)
    ...
    if (precw == 2 && precr == 4) 
        [d, err, iter, ~] = pcg_dq_lsq(A,zeros(n,1),double(r),LL,gtol);
    elseif (precw == 1 && precr == 2)
        [d, err, iter, ~] = pcg_sd_lsq(A,single(zeros(n,1)),single(r),LL,gtol);
    elseif (precw == 2 && precr == 2)
        [d, err, iter, ~] = pcg_dd_lsq(A,zeros(n,1),double(r),LL,gtol);
    end
    
    t_pcg_its = t_pcg_its+iter;
